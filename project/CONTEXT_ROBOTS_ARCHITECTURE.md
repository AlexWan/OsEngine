# CONTEXT_ROBOTS_ARCHITECTURE — Архитектура торговых роботов OsEngine

> Общая карта всех контекстов проекта — в файле [`CONTEXT.md`](CONTEXT.md).

## 1. Архитектура роботов

### 1.1 Базовый класс: `BotPanel`
...
3. **Конструктор робота.** Конструктор всегда должен иметь сигнатуру:
   ...
   - `startProgram` — тип запуска (`IsOsTrader`, `IsTester`, `IsOsOptimizer`)
...
### 1.2 Типы торговых табов (`BotTabType`)

`IIBotTab` — это базовый интерфейс торгового источника. Это источник всех данных (свечи, тики, стакан, позиции) и единственный способ совершать торговые операции.
...
### 1.3 События таба и торговые операции
...
### 1.4 Необязательные члены `BotPanel`
...
### 2.2 Карта навигации: от задачи к папке
...
### 3.3 Как составить промпт на робота

Готовые шаблоны промптов и подробная структура — в `CONTEXT_PROMPTS_ROBOTS.md`.

Кратко: перед созданием робота составьте запрос, содержащий тип таба, индикаторы, точки входа/выхода, пример для заимствования из `CONTEXT_ROBOTS.md`, параметры и особые случаи.

---

## 4. Быстрые шаблоны

### 4.1 Минимальный робот на `BotTabSimple`
...
### 4.2 Минимальный скринер на `BotTabScreener`
...
### 4.3 Минимальный парный арбитраж на `BotTabPair`
