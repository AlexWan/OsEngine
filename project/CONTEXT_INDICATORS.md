...
Базовая архитектура роботов — в `CONTEXT.md`.
...
|--------|------|------------------|
| Внутри проекта | `OsEngine/Indicators/Scripts/MyInd.cs` | Если вы работаете в репозитории и собираете проект из исходников |
| Вне проекта (runtime) | `Custom/Indicators/Scripts/MyInd.cs` (рядом с `.exe`) | Если пользователь кладёт файл рядом с собранным терминалом |
| DLL | `Custom/Indicators/Scripts/Dlls/MyLib.dll` | Внешние зависимости для скриптов |

**Правила размещения в `OsEngine/Indicators/Scripts/`:**
1. Файл: `MyInd.cs`
...
4. Пространство имён: любое
5. Компиляция: не требуется — индикатор автоматически подхватывается фабрикой при первом обращении
