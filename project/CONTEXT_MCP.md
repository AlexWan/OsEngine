...
OsEngine/MCP/
  ...
  Modules/                  // Обработчики инструментов
    ...
    WikiDividendsApi.cs     // wiki_dividends_get_history, wiki_dividends_get_future, wiki_dividends_get_nearest, wiki_dividends_search_by_date
    OsDataApi.cs            // data_*
...
| `wiki_dividends_get_future` | Будущие дивиденды российской акции из `Wiki/Dividends/{ticker}.md`. Параметры: `ticker` (обязательный), `date` (dd.MM.yyyy, по умолчанию сегодня), `refresh`. Возвращает записи с `registry_close_date >= date` |
| `wiki_dividends_get_nearest` | Ближайшая предстоящая дата закрытия реестра от заданной даты. Параметры: `ticker`, `from_date` (dd.MM.yyyy, по умолчанию сегодня), `refresh` |
...
**tools/call (wiki_dividends_get_nearest):**

```bash
curl -s -H "X-Api-Key: osengine-mcp-default-key" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"wiki_dividends_get_nearest","arguments":{"ticker":"SBER","from_date":"15.03.2024"}},"id":30}' \
  http://localhost:6500/api/v1/mcp
```
