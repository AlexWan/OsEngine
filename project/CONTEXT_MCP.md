...
| `server_get_list` | Список развёрнутых биржевых коннекторов (имя, тип, статус) |
| `server_activate` | Активация сервера указанного типа: загрузка сохранённых экземпляров и создание основного |
| `server_get_params` | Получить параметры конкретного экземпляра сервера. Пароли маскируются |
...
**tools/call (server_get_list):**

```bash
curl -s -H "X-Api-Key: osengine-mcp-default-key" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"server_get_list","arguments":{}},"id":11}' \
  http://localhost:6500/api/v1/mcp
```

**Пример ответа `server_get_list`:**
...
**tools/call (server_activate):**

```bash
curl -s -H "X-Api-Key: osengine-mcp-default-key" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"server_activate","arguments":{"type":"TInvest"}},"id":12}' \
  http://localhost:6500/api/v1/mcp
```

**Пример ответа `server_activate`:**
...
**tools/call (server_get_params):**

```bash
curl -s -H "X-Api-Key: osengine-mcp-default-key" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"server_get_params","arguments":{"type":"TInvest","number":0}},"id":13}' \
  http://localhost:6500/api/v1/mcp
```

**Пример ответа `server_get_params`:**
...
| Server | `server_get_list`, `server_activate`, `server_get_params` |
