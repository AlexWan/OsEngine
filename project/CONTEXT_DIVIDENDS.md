...
- Выплаты начисляются открытым позициям на первый торговый день после даты Т-1.
- Позиция должна быть открыта до или в дату Т-1.
- Для каждого дивиденда создаётся синтетическая позиция `<тикер>_divs` в журнале робота.
...
- Синтетические позиции `<тикер>_divs` создаются в журнале бота; сумма выплаты рассчитывается с учётом НДФЛ **13%**.
