...
- **Режимы:**
  - **Классический:** ребалансировка между дивидендными акциями (с фильтром по SMA) и золотом по заданным весам.
  - **Дивидендный:** при наличии акций с ближайшими дивидендами весь депозит входит в эти акции равномерно.
- **Дивидендный фильтр:**
  - будущие дивиденды через `WikiMaster.GetDividendsFuture` для дивидендного режима;
  - прошлые дивиденды через `WikiMaster.GetDividendsPast` (до 380 дней) для классического режима;
  - минимальная дивидендная доходность `Min dividend yield percent`.
- **Особенности:**
  - fallback-набор тикеров (SBER / SBERP / GAZP / LKOH / VTBR) в тестере при отсутствии дивидендных данных;
  - умный ребаланс в реальном режиме через `BuyAtMarketToPosition` / `CloseAtMarket`.
...
Отдельных страниц для привилегированных акций на Smart-Lab обычно нет: `/q/SBERP/dividend/` отдаёт 404, а `/q/SNGSP/dividend/` возвращает 200 со страницей ошибки без таблицы. Строки префа лежат в общей таблице на странице базовой бумаги, первый столбец таблицы — тикер. Поэтому если на странице тикера не нашлось ни одной строки с этим тикером и тикер оканчивается на `P`, апдейтер повторяет запрос к странице базовой бумаги (`SBERP` → `SBER`) и выбирает из таблицы только строки своего тикера. Дивиденд и даты у префа и обычки совпадают, доходность различается (считается от цены конкретной бумаги) — в файл попадает доходность из строки префа. Поле `Source` в метаданных файла указывает фактически использованную страницу (для SBERP — `q/SBER/dividend/`). Часть префов не покрыта Smart-Lab вовсе (на странице базовой бумаги их строк тоже нет) — такие тикеры остаются без файла.
...
- Синтетическая позиция `<тикер>_divs` в журнале робота создаётся на первый торговый день после даты Т-1. Она направленная: для лонга закрывается с профитом, для шорта — с убытком на размер дивиденда.
- Запись в таблицу выплат тестера и движение денег по виртуальному портфелю происходят с задержкой **7 календарных дней** после Т-1 и учитывают направление позиции: лонгу — начисление (плюс в портфель и в таблицу), шорту — **списание** (минус).
- Позиция должна быть открыта до или в дату Т-1.
- Начисление выполняется для каждой открытой позиции каждого робота независимо: ключ дедупликации — `робот + таб + номер позиции + тикер + дата`. Несколько роботов в одной бумаге (в т.ч. лонг и шорт одновременно) и сеточные роботы с несколькими позициями обрабатываются корректно.
- Сумма выплаты и доходность синтетической позиции рассчитываются с учётом НДФЛ **13%** (брокер удерживает налог при зачислении дивиденда). Для шорта списание считается по тому же коэффициенту.
- В таблице отображаются: бумага, дата выплаты, сумма (со знаком), имя робота.
...
- Синтетические позиции `<тикер>_divs` создаются в журнале бота на первый торговый день после Т-1 (направленные: для шорта — с убытком). Движение по портфелю происходит с задержкой **7 календарных дней** после Т-1: лонгу — начисление, шорту — **списание**. Начисление выполняется для каждой позиции независимо (ключ: бот + таб + номер позиции + тикер + дата). Сумма выплаты рассчитывается с учётом НДФЛ **13%**.
