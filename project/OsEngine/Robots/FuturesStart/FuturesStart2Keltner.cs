...
Трендовушка на пробое канала Келтнера. С фильтром по стадии отклонения фьючерса от базы.
...
ВХОД в позицию
Пересечение верхней или нижней линии канала

Выход из позиции
Пересечение обратной стороны канала
...
{
    [Bot("FuturesStart2Keltner")]
    public class FuturesStart2Keltner : BotPanel
    {
        BotTabSimple _base1;
        ...
        public FuturesStart2Keltner(string name, StartProgram startProgram) : base(name, startProgram)
        {
            // non trade periods
            ...
            "Eng:Trend futures screener on the Keltner channel breakout. With a filter by the stage of the futures deviation from the base. Designed for the MOEX stock futures market_" +
            "Ru:Трендовый скринер фьючерсов на пробое канала Келтнера. С фильтром по стадии отклонения фьючерса от базы. Рассчитана на рынок фьючерсов на акции MOEX_");
        ...
        {
            futuresSource.CreateCandleIndicator(1, "KeltnerChannel", new List<string>() {
            ...
            Position pos)
        {
            ...
            if (pos.Direction == Side.Sell
                && futuresLastPrice > keltner.DataSeries[1].Last)
            {
                needToExit = true;
            }
