    ...
    public class MoexSpreadScreener : BotPanel
    {
        ...
        private void SetSpreadInTable(List<Candle> candles, BotTabSimple tab)
        {
            ...
            for (int i = 0; i < _grid.Rows.Count - 1; i++)
            {
                ...
                {
                    _grid.Rows[i].Cells[4].Value = GetSpread(Convert.ToDecimal(_grid.Rows[i].Cells[2].Value), Convert.ToDecimal(_grid.Rows[i].Cells[3].Value));                   
                }
            }

        }

        private object GetSpread(decimal price1, decimal price2)
        {
        ...
        private void CreateButtonAddRow()
        {
            ...
            _grid.Rows[_grid.Rows.Count - 1].Cells[4].ReadOnly = true;

        }

        private void NewGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 5 && e.RowIndex != _grid.Rows.Count - 1)
            {
