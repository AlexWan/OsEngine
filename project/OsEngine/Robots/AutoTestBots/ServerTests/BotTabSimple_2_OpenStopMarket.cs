...
{
    /// <summary>
    /// Test for BotTabSimple.BuyAtStopMarketOnServer / SellAtStopMarketOnServer (server StopMarket position opening)
    /// </summary>
    public class BotTabSimple_2_OpenStopMarket : AServerTester
    {
        ...
        public static string TestDescription
        {
            ...
            {
                string messageEng =
                    "Test B2. Opening long and short positions by server StopMarket orders via BotTabSimple methods BuyAtStopMarketOnServer and SellAtStopMarketOnServer, with and without signalType. Checks order fields, position signal, stop trigger mechanics with a spawned child Market order and trades on full volume. The test creates its own BotTabSimple tab and deletes it at the end.\n\n" +
                    "Parameters\n" +
                    ...
                    "Checks in order\n" +
                    "1. Protective branches. Zero volume and null position must not create positions or orders.\n" +
                    "2. Wait for best bid and ask from the tab connector.\n" +
                    "3. Long. Buy StopMarket above the best ask, overload with signalType. A position is created, direction Buy, SignalTypeOpen equals the signal. Open order checks - type StopMarket, IsStopOrProfit flag, StopPrice, Volume, condition Open.\n" +
                    "4. The stop order becomes Active. If it is rejected, it is re-placed with fresh prices.\n" +
                    "5. Wait for the stop trigger. The stop goes to Cancel, a child Market order appears with a link to the stop, the stop links back to the child. If there is no trigger in time, the stop is cancelled and re-placed.\n" +
                    "6. The position is opened on full volume. MyTrades arrived on full volume, their fields are valid.\n" +
                    "7. Short. Steps 3-6 are repeated by a Sell StopMarket below the best bid, overload without signalType.\n" +
                ...
                string messageRu =
                    "Тест B2. Открытие длинной и короткой позиций серверными StopMarket ордерами через методы BotTabSimple BuyAtStopMarketOnServer и SellAtStopMarketOnServer, с signalType и без. Проверяются поля ордеров, сигнал позиции, механика срабатывания стопа с порождением дочернего Market ордера и сделки на полный объём. Тест создаёт собственную вкладку BotTabSimple и удаляет её в конце.\n\n" +
                    "Параметры\n" +
                    ...
                    "Проверки по порядку\n" +
                    "1. Защитные ветки. Нулевой объём и null позиция не должны создавать позиции и ордера.\n" +
                    "2. Ожидание лучших бида и аска от коннектора вкладки.\n" +
                    "3. Лонг. Buy StopMarket выше лучшего аска, перегрузка с signalType. Создана позиция, направление Buy, SignalTypeOpen равен сигналу. Проверки открывающего ордера - тип StopMarket, флаг IsStopOrProfit, StopPrice, объём, условие Open.\n" +
                    "4. Стоп-ордер переходит в Active. Если он отклонён, он перевыставляется по свежим ценам.\n" +
                    "5. Ожидание срабатывания стопа. Стоп переходит в Cancel, появляется дочерний Market ордер со ссылкой на стоп, стоп ссылается обратно на дочерний. Если срабатывания нет вовремя, стоп отменяется и перевыставляется.\n" +
                    "6. Позиция открыта на полный объём. Пришли MyTrades на полный объём, их поля валидны.\n" +
                    "7. Шорт. Шаги 3-6 повторяются через Sell StopMarket ниже лучшего бида, перегрузка без signalType.\n" +
        ...
        {
            InitFileLog();
            Log("TEST START. Methods: BuyAtStopMarketOnServer / SellAtStopMarketOnServer (StopMarket open)");
            ...
            try
            {
                ...
                Log("[Step 1/4] Protective branches: zero volume, null position");

                Tab.BuyAtStopMarketOnServer(0, 1);
                Tab.SellAtStopMarketOnServer(0, 1);
                ...
                // Step 3. Long by server StopMarket. Overload with signalType

                Log("[Step 3/4] BuyAtStopMarketOnServer with signalType. Long");

                if (OpenByStopMarket(Side.Buy, sec, "TestSignalOpen") == false)
                {
                    return;
                }

                // Step 4. Short by server StopMarket. Overload without signalType

                Log("[Step 4/4] SellAtStopMarketOnServer without signalType. Short");

                if (OpenByStopMarket(Side.Sell, sec, null) == false)
                {
                    return;
                }

                SetNewServiceInfo("BuyAtStopMarketOnServer and SellAtStopMarketOnServer: positions opened, all checks passed");
            }
            finally
            {
                Teardown();
        ...
        private bool OpenByStopMarket(Side side, Security sec, string signalType)
        {
            for (int attempt = 0; attempt < Attempts; attempt++)
            {
                ...
                Log("Attempt " + (attempt + 1) + ". " + side
                    + " StopMarket. Activation: " + activation
                    + " Volume: " + VolumeToTrade
                    + " Signal: '" + (signalType == null ? "none" : signalType) + "'");
                ...
                if (side == Side.Buy)
                {
                    if (signalType == null)
                    {
                        Tab.BuyAtStopMarketOnServer(VolumeToTrade, activation);
                    }
                    else
                    {
                        Tab.BuyAtStopMarketOnServer(VolumeToTrade, activation, signalType);
                    }
                }
                else
                {
                    if (signalType == null)
                    {
                        Tab.SellAtStopMarketOnServer(VolumeToTrade, activation);
                    }
                    else
                    {
                        Tab.SellAtStopMarketOnServer(VolumeToTrade, activation, signalType);
                    }
                }
                ...
                {
                    SetNewError("Error 20. No position created after " + side + "AtStopMarketOnServer");
                    return false;
                }

                Position pos = Tab.PositionsLast;
                TrackPosition(pos);
                DumpPosition(pos, "after " + side + "AtStopMarketOnServer. Attempt " + (attempt + 1));
                ...
                if (signalType != null
                    && pos.SignalTypeOpen != signalType)
                {
                    SetNewError("Error 22. Position SignalTypeOpen is wrong. Expected: '" + signalType
                        + "' Real: '" + pos.SignalTypeOpen + "'");
                    return false;
                }
                ...
                {
                    SetNewError("Error 23. Position has no open orders");
                    return false;
                }
                ...
                {
                    if (pos.OpenOrders[i].TypeOrder == OrderPriceType.StopMarket
                ...
                if (order == null)
                {
                    SetNewError("Error 33. No StopMarket server stop in position open orders");
                    return false;
                }
                ...
                {
                    SetNewError("Error 24. Open order type is not StopMarket. Real: " + order.TypeOrder);
                    return false;
                }

                if (order.IsStopOrProfit == false)
                {
                    SetNewError("Error 25. Open order IsStopOrProfit == false");
                    return false;
                }
                ...
                {
                    SetNewError("Error 26. Order StopPrice is wrong. Expected: " + expectedStop
                ...
                {
                    Log(side + " stopMarket triggered during the pause after placement. Already Cancel");
                }
                else if (WaitOrderActive(order, side + " stopMarket open") == false)
                {
                    SetNewError("Error 29. No Active state from " + side + " stopMarket order");
                    return false;
                }

                if (order.State == OrderStateType.Fail)
                {
                    Log(side + " stopMarket order was rejected. Re-place with fresh prices");
                    continue;
                }

                Log(side + " stopMarket order Active. NumMarket: " + order.NumberMarket);

                if (OrderIsNormal(order, side, OrderPriceType.StopMarket) == false)
                {
                    return false;
                }

                // wait trigger: the stop goes Cancel and spawns a child market order

                bool triggered = WaitFor(() =>
                    order.State == OrderStateType.Cancel
                    || pos.State == PositionStateType.Open
                    || pos.OpenVolume > 0,
                    WaitTriggerSeconds, side + " wait stopMarket trigger");

                if (triggered == false)
                {
                    Log(side + " stopMarket was not triggered in time. Cancel and re-place");
                ...
                if (CheckStopTriggerWithChild(order, side, OrderPriceType.Market,
                    side + " StopMarket open") == false)
                {
                    return false;
                }

                if (WaitPositionOpen(pos, side + " stopMarket open") == false)
                {
                    SetNewError("Error 30. Position was not opened after stopMarket trigger. " + side);
                    return false;
                }

                // the child order may fill partially: wait for the full volume
                ...
                SetNewServiceInfo(side + "AtStopMarketOnServer: position opened by server StopMarket. Checks passed");

                return true;
            }

            SetNewError("Error 32. " + side + "AtStopMarketOnServer. No triggered stop after " + Attempts + " attempts");
            return false;
        }
