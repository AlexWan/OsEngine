...
{
    internal class DataTest1_IntegrityOfData : AServerTester
    {
        ...
        private void StartTestSecurity(Security security)
        {
            ...
            if(permission == null)
            {
                SetNewError("Data1. Error 0. No server permission to server. Type: " + Server.ServerType.ToString());
                return;
            }
        ...
        private void CheckCandleData(TimeFrame timeFrame,Security security,DateTime startTime, DateTime endTime)
        {
            ...
            {
                SetNewError("Data1. Error 2. Exception on server request. " + timeFrame.ToString() + "\n" + ex.ToString());
                return;
            }

            if(candles == null)
            {
                SetNewError("Data1. Error 3. Null in array. " + timeFrame.ToString());
                return;
            }

            if (candles.Count == 0)
            {
                SetNewError("Data1. Error 4. Zero count elements in array. " + timeFrame.ToString());
                return;
            }
            ...
            if(daysCount != 2)
            {
                SetNewError("Data1. Error 5. Days in candles is not 2. " + timeFrame.ToString() + "\n" 
        ...
        private void CheckTradeData(TimeFrame timeFrame, Security security, DateTime startTime, DateTime endTime)
        {
            ...
            {
                SetNewError("Data1. Error 6. Exception on server request. " + timeFrame.ToString() + "\n" + ex.ToString());
                return;
            }

            if (trades == null)
            {
                SetNewError("Data1. Error 7. Null in array. " + timeFrame.ToString());
                return;
            }

            if (trades.Count == 0)
            {
                SetNewError("Data1. Error 8. Zero count elements in array. " + timeFrame.ToString());
                return;
            }
            ...
            if (daysCount != 2)
            {
                SetNewError("Data1. Error 9. Days in trades is not 2. " + timeFrame.ToString() + "\n"
                    + " StartTime: " + startTimeReal.ToString()
                    + " EndTime: " + endTimeReal.ToString());
            }
        }
