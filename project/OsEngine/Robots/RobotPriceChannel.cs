...
namespace OsEngine.Robots.RobotPriceChannel
{
    ...
    class RobotPriceChannel : BotPanel
    {
        ...
        public RobotPriceChannel(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            VolumeRegime = CreateParameter("Volume type", "Contract currency", new[] { "Number of contracts", "Contract currency" });
            VolumeDecimals = CreateParameter("Number of Digits after the decimal point in the volume", 2, 1, 50, 4);
            VolumeOnPosition = CreateParameter("Volume", 10, 1.0m, 50, 4);
            Slippage = CreateParameter("Slippage %", 0m, 0, 20, 1 );
