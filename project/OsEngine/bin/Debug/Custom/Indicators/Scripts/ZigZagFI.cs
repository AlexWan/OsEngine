...
{
    internal class ZigZagFI : Aindicator
    {
        private Aindicator _FI;

        private IndicatorDataSeries _seriesFI;

        private IndicatorParameterInt _periodFI;
        ...
        public override void OnStateChange(IndicatorState state)
        {
            ...
            {
                _periodFI = CreateParameterInt("period ForceIndex", 13);

                _lengthZigZag = CreateParameterInt("Length ZigZag", 14);

                _seriesFI = CreateSeries("ForceIndex", Color.Aqua, IndicatorChartPaintType.Line, true);
                ...
                ((IndicatorParameterInt)_FI.Parameters[0]).Bind(_periodFI);
                ProcessIndicator("ForceIndex", _FI);
        ...
        {
            _seriesFI.Values[index] = _FI.DataSeries[0].Values[index];

            List<decimal> values = _FI.DataSeries[0].Values;
            ...
            List<decimal> valuesFI = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesFI.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesFI[valuesFI.Count - 1];
            Low = valuesFI[valuesFI.Count - 1];
