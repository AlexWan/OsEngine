...
{
    internal class ZigZagRVI : Aindicator
    {
        ...
        {
            _seriesRVI.Values[index] = _RVI.DataSeries[1].Values[index];

            List<decimal> values = _RVI.DataSeries[1].Values;
            ...
            List<decimal> valuesRVI = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesRVI.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesRVI[valuesRVI.Count - 1];
            Low = valuesRVI[valuesRVI.Count - 1];
