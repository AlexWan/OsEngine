...
{
    internal class ZigZagCCI : Aindicator
    {
        ...
        {
            _seriesCCI.Values[index] = _CCI.DataSeries[0].Values[index];

            List<decimal> values = _CCI.DataSeries[0].Values;
            ...
            List<decimal> valuesCCI = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesCCI.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesCCI[valuesCCI.Count - 1];
            Low = valuesCCI[valuesCCI.Count - 1];
