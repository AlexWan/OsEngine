...
{
    internal class ZigZagAD : Aindicator
    {
        ...
        public override void OnStateChange(IndicatorState state)
        {
            if (state == IndicatorState.Configure)
            {
                ...
                _seriesAD = CreateSeries("AccumulationDistribution", Color.Aqua, IndicatorChartPaintType.Line, true);
                ...
                ProcessIndicator("AccumulationDistribution", _AD);
        ...
        {
            _seriesAD.Values[index] = _AD.DataSeries[0].Values[index];
            ...
            List<decimal> valuesAD = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesAD.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesAD[valuesAD.Count - 1];
            Low = valuesAD[valuesAD.Count - 1];
