...
namespace OsEngine.Indicators
{
    ...
    public class ZigZagMACD : Aindicator
    {
        ...
        {
            _seriesMACD.Values[index] = _MACD.DataSeries[0].Values[index];

            List<decimal> values = _MACD.DataSeries[0].Values;
            ...
            List<decimal> valuesMACD = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {

                valuesMACD.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;
            if (valuesMACD[valuesMACD.Count - 1] > 0)
            {
                High = valuesMACD[valuesMACD.Count - 1];
                Low = 0;
            }
            else
            {
                High = 0;
                Low = valuesMACD[valuesMACD.Count - 1];
            }
