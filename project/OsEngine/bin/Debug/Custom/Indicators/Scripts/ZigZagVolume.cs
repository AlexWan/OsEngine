    ...
    public class ZigZagVolume : Aindicator
    {
        ...
        public override void OnStateChange(IndicatorState state)
        {
            if (state == IndicatorState.Configure)
            {
                ...
                _seriesVolume = CreateSeries("VolumeOscilator", Color.Aqua, IndicatorChartPaintType.Line, true);
        ...
        public override void OnProcess(List<Candle> candles, int index)
        {
            ...
            List<decimal> valuesVolume = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesVolume.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesVolume[valuesVolume.Count - 1];
            Low = valuesVolume[valuesVolume.Count - 1];
