...
{
    internal class ZigZagMomentum : Aindicator
    {
        ...
        {
            _seriesMomentum.Values[index] = _Momentum.DataSeries[0].Values[index];
            ...
            List<decimal> valuesMomentum = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesMomentum.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesMomentum[valuesMomentum.Count - 1];
            Low = valuesMomentum[valuesMomentum.Count - 1];
