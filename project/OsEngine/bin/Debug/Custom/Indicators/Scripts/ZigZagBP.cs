...
{
    internal class ZigZagBP : Aindicator
    {
        private Aindicator _bP;
        private IndicatorParameterInt _lengthBP;
        private IndicatorDataSeries _seriesBP;
        ...
        public override void OnStateChange(IndicatorState state)
        {
            if (state == IndicatorState.Configure)
            {
                ...
                _lengthBP = CreateParameterInt("Period", 13);
                _lengthZigZag = CreateParameterInt("Length ZigZag", 14);

                _seriesBP = CreateSeries("Values", Color.OrangeRed, IndicatorChartPaintType.Column, true);
                ...
                _bP = IndicatorsFactory.CreateIndicatorByName("BearsPower", Name + "Bears Power", false);
                ((IndicatorParameterInt)_bP.Parameters[0]).Bind(_lengthBP);                           
                ProcessIndicator("AO", _bP);
        ...
        {
            _seriesBP.Values[index] = _bP.DataSeries[0].Values[index];
         
            List<decimal> values = _bP.DataSeries[0].Values;
            ...
            if (index < _lengthBP.ValueInt)
            {
                return;
            }

            List<decimal> valuesBP = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
              
                valuesBP.Add(values[i]);
            }
            
            decimal High = 0;
            decimal Low = 0;
            if (valuesBP[valuesBP.Count - 1] > 0)
            {
                High = valuesBP[valuesBP.Count - 1];
                Low = 0;
            }
            else
            {
                High = 0;
                Low = valuesBP[valuesBP.Count - 1];
            }
