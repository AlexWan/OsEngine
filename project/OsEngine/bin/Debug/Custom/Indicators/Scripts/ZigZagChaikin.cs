...
{
    internal class ZigZagChaikin : Aindicator
    {
        ...
        public override void OnStateChange(IndicatorState state)
        {
            if (state == IndicatorState.Configure)
            {
                ...
                _seriesChaikin = CreateSeries("ChaikinOsc", Color.Aqua, IndicatorChartPaintType.Line, true);
        ...
        {
            _seriesChaikin.Values[index] = _ChaikinOsc.DataSeries[0].Values[index];

            List<decimal> values = _ChaikinOsc.DataSeries[0].Values;
            ...
            List<decimal> valuesChaikin = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesChaikin.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesChaikin[valuesChaikin.Count - 1];
            Low = valuesChaikin[valuesChaikin.Count - 1];
