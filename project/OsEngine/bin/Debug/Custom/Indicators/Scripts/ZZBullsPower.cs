...
namespace OsEngine.Indicators
{
    ...
    public class ZZBullsPower : Aindicator
    {
        ...
        private IndicatorDataSeries _seriesBullsPower;
        ...
        public override void OnStateChange(IndicatorState state)
        {
            if (state == IndicatorState.Configure)
            {
                ...
                _lengthBullsPower = CreateParameterInt("Period", 13);
                _lengthZigZag = CreateParameterInt("Length ZigZag", 14);

                _seriesBullsPower = CreateSeries("Values", Color.OrangeRed, IndicatorChartPaintType.Column, true);
                ...
                _bullsPower = IndicatorsFactory.CreateIndicatorByName("BullsPower", Name + "Bulls Power", false);
                ((IndicatorParameterInt)_bullsPower.Parameters[0]).Bind(_lengthBullsPower);
                ProcessIndicator("BullsPower", _bullsPower);
        ...
        {
            _seriesBullsPower.Values[index] = _bullsPower.DataSeries[0].Values[index];

            List<decimal> values = _bullsPower.DataSeries[0].Values;
            ...
            if (index < _lengthBullsPower.ValueInt)
            {
                return;
            }

            List<decimal> valuesBullsPower = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {

                valuesBullsPower.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;
            if (valuesBullsPower[valuesBullsPower.Count - 1] > 0)
            {
                High = valuesBullsPower[valuesBullsPower.Count - 1];
                Low = 0;
            }
            else
            {
                High = 0;
                Low = valuesBullsPower[valuesBullsPower.Count - 1];
            }
