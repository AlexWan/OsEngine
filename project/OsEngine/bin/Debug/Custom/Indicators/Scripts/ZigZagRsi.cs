...
{
    internal class ZigZagRsi : Aindicator
    {
        private Aindicator _Rsi;

        private IndicatorDataSeries _seriesRsi;

        private IndicatorParameterInt _periodRsi;
        ...
        public override void OnStateChange(IndicatorState state)
        {
            ...
            {
                _periodRsi = CreateParameterInt("period Rsi", 14);

                _lengthZigZag = CreateParameterInt("Length ZigZag", 14);

                _seriesRsi = CreateSeries("RSI", Color.Aqua, IndicatorChartPaintType.Line, true);
                ...
                ((IndicatorParameterInt)_Rsi.Parameters[0]).Bind(_periodRsi);
        ...
        public override void OnProcess(List<Candle> candles, int index)
        {
            ...
            List<decimal> valuesRSI = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesRSI.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesRSI[valuesRSI.Count - 1];
            Low = valuesRSI[valuesRSI.Count - 1];
