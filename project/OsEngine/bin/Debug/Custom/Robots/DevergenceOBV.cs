...
The trend robot on strategy Devergence OBV.
...
{
    [Bot("DevergenceOBV")] // We create an attribute so that we don't write anything to the BotFactory
    public class DevergenceOBV : BotPanel
    {
        ...
        private Aindicator _zigZagOBV;

        // Exit settings
        private StrategyParameterDecimal _stopValue;
        private StrategyParameterDecimal _profitValue;

        public DevergenceOBV(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            // Create indicator ZigZag OBV
            _zigZagOBV = IndicatorsFactory.CreateIndicatorByName("ZigZagOBV", name + "ZigZagOBV", false);
            _zigZagOBV = (Aindicator)_tab.CreateCandleIndicator(_zigZagOBV, "NewArea");
            ((IndicatorParameterInt)_zigZagOBV.Parameters[0]).ValueInt = _periodZigZag.ValueInt;
            _zigZagOBV.Save();
            ...
            ParametrsChangeByUser += DevergenceOBV_ParametrsChangeByUser; ;

            // Subscribe to the candle finished event
            _tab.CandleFinishedEvent += _tab_CandleFinishedEvent;

            Description = OsLocalization.Description.DescriptionLabel303;
        }

        private void DevergenceOBV_ParametrsChangeByUser()
        {
            ((IndicatorParameterInt)_zigZag.Parameters[0]).ValueInt = _periodZigZag.ValueInt;
            _zigZag.Save();
            _zigZag.Reload();
        ...
        private void LogicOpenPosition(List<Candle> candles)
        {
            ...
            if (openPositions == null || openPositions.Count == 0)
            {
                ...
                List<decimal> zzAOLow = _zigZagOBV.DataSeries[4].Values;
                List<decimal> zzAOHigh = _zigZagOBV.DataSeries[3].Values;
