    ...
    public class StrategyStdDevSmaAndRsi : BotPanel
    {
        ...
        private void LogicOpenPosition(List<Candle> candles)
        {
            ...
            if (openPositions == null || openPositions.Count == 0)
            {
                ...
                {
                    if (_lastRSI > 50 && _lastSD > _minValue.ValueDecimal && _lastSma < lastPrice)
                    {
                        if (_orderType == "Limit")
                        {
                            _tab.BuyAtLimit(GetVolume(_tab), _tab.PriceBestAsk + _slippage, candles.Last().TimeStart.ToString());
                        }
                        else
                        {
                            _tab.BuyAtMarket(GetVolume(_tab), candles.Last().TimeStart.ToString());
                        }
                    }
                }
                ...
                if (_regime.ValueString != "OnlyLong") // If the mode is not only long, then we enter short
                {
                    if (_lastRSI < 50 && _lastSD > _minValue.ValueDecimal && _lastSma > lastPrice)
                    {
                        if (_orderType == "Limit")
                        {
                            _tab.SellAtLimit(GetVolume(_tab), _tab.PriceBestBid - _slippage, candles.Last().TimeStart.ToString());
                        }
                        else
                        {
                            _tab.SellAtMarket(GetVolume(_tab), candles.Last().TimeStart.ToString());
                        }
                    }
                }
            }
