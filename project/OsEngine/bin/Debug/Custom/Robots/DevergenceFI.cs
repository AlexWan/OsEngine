...
The trend robot on strategy Devergence ForceIndex.
...
{
    [Bot("DevergenceFI")] // We create an attribute so that we don't write anything to the BotFactory
    public class DevergenceFI : BotPanel
    {
        ...
        private StrategyParameterInt _periodFI;

        // Indicator
        private Aindicator _zigZag;
        private Aindicator _zigZagFI;

        // Exit settings
        private StrategyParameterInt _stopValue;
        private StrategyParameterInt _profitValue;

        public DevergenceFI(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            _periodFI = CreateParameter("Period FI", 10, 10, 300, 10, "Indicator");
            ...
            // Create indicator ZigZag FI
            _zigZagFI = IndicatorsFactory.CreateIndicatorByName("ZigZagFI", name + "ZigZagFI", false);
            _zigZagFI = (Aindicator)_tab.CreateCandleIndicator(_zigZagFI, "NewArea");
            ((IndicatorParameterInt)_zigZagFI.Parameters[0]).ValueInt = _periodFI.ValueInt;
            ((IndicatorParameterInt)_zigZagFI.Parameters[1]).ValueInt = _periodZigZag.ValueInt;
            _zigZagFI.Save();

            // Exit settings
            _stopValue = CreateParameter("Stop Value", 1, 5, 200, 5, "Exit");
            _profitValue = CreateParameter("Profit Value", 1, 5, 200, 5, "Exit");

            // Subscribe to the indicator update event
            ParametrsChangeByUser += DevergenceFI_ParametrsChangeByUser; ;
        ...
        private void _tab_CandleFinishedEvent(List<Candle> candles)
        {
            ...
            if (candles.Count < _periodFI.ValueInt || candles.Count < _periodZigZag.ValueInt)
            {
                return;
            }
        ...
        private void LogicOpenPosition(List<Candle> candles)
        {
            ...
            if (openPositions == null || openPositions.Count == 0)
            {
                ...
                List<decimal> zzAOLow = _zigZagFI.DataSeries[4].Values;
                List<decimal> zzAOHigh = _zigZagFI.DataSeries[3].Values;
        ...
        private void LogicClosePosition(List<Candle> candles)
        {
            ...
            for (int i = 0; openPositions != null && i < openPositions.Count; i++)
            {
                ...
                {
                    decimal profitActivation = pos.EntryPrice + pos.EntryPrice * _profitValue.ValueInt / 100;
                    decimal stopActivation = pos.EntryPrice - pos.EntryPrice * _stopValue.ValueInt / 100;
