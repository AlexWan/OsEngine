...
namespace OsEngine.Robots
{
    ...
    public class CountertrendEmaATR : BotPanel
    {
        ...
        private StrategyParameterInt _candlesCountAtr;
        private StrategyParameterInt _volumeValue;
        ...
        public CountertrendEmaATR(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            // Indicator Settings
            _periodEma = CreateParameter("Period Ema", 50, 10, 300, 10,"Indicator");
            _candlesCountAtr = CreateParameter("Candles Count Atr", 21, 7, 48, 7, "Indicator");
            _lengthAtr = CreateParameter("Length ATR", 14, 7, 48, 7, "Indicator");
            _multAtr = CreateParameter("Mult ATR", 1.1m, 1.1m, 10, 0.1m, "Indicator");
            _volumeValue = CreateParameter("Volume Value", 1000, 100, 10000, 100, "Indicator");

            // Create indicator Volume
            _volumeIndicator = IndicatorsFactory.CreateIndicatorByName("Volume", name + "Vol", false);
            ...
            Description = OsLocalization.Description.DescriptionLabel188;
        }
        ...
        private void _tab_CandleFinishedEvent(List<Candle> candles)
        {
            ...
            if (candles.Count < _lengthAtr.ValueInt ||
                candles.Count < _periodEma.ValueInt ||
                candles.Count < _candlesCountAtr.ValueInt)
            {
                return;
            }
        ...
        private void LogicOpenPosition(List<Candle> candles)
        {
            ...
            // The prev value of the indicator
            _prevVolume = _volumeIndicator.DataSeries[0].Values[_volumeIndicator.DataSeries[0].Values.Count - 2];
            ...
            if (openPositions == null || openPositions.Count == 0)
            {
                ...
                {
                    if (_volumeValue.ValueInt < _lastVolume && 
                        candles[candles.Count - 1].IsDown && 
                        _lastEma > lastPrice &&
                        _lastATR > AverageVolumeValue(Volume,_candlesCountAtr.ValueInt) * _multAtr.ValueDecimal)
                    {
