            ...
            if (openPositions == null || openPositions.Count == 0)
            {
                ...
                if (_regime.ValueString != "OnlyShort") // If the mode is not only short, then we enter long
                {
                    if (lastCog > _entryLevel.ValueDecimal && lastRSI > 50)
                    {
                        ...
                        {
                            _tab.BuyAtStopMarket(GetVolume(_tab), upChannel + _slippage, upChannel, StopActivateType.HigherOrEqual, 1, "", PositionOpenerToStopLifeTimeType.NoLifeTime);
                        }
                    }
                }
                ...
                if (_regime.ValueString != "OnlyLong") // If the mode is not only long, then we enter short
                {
                    if (lastCog < _entryLevel.ValueDecimal && lastRSI < 50)
                    {
                        ...
                        {
                            _tab.SellAtStopMarket(GetVolume(_tab), downChannel - _slippage, downChannel, StopActivateType.LowerOrEqual, 1, "", PositionOpenerToStopLifeTimeType.NoLifeTime);
                        }
                    }
                }
            }
