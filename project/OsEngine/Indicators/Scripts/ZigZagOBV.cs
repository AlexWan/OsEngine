    ...
    public class ZigZagOBV : Aindicator
    {
        ...
        public override void OnStateChange(IndicatorState state)
        {
            if (state == IndicatorState.Configure)
            {
                ...
                _seriesOBV = CreateSeries("OBV", Color.Aqua, IndicatorChartPaintType.Line, true);
        ...
        {
            _seriesOBV.Values[index] = _OBV.DataSeries[0].Values[index];
            ...
            List<decimal> valuesOBV = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesOBV.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesOBV[valuesOBV.Count - 1];
            Low = valuesOBV[valuesOBV.Count - 1];
