...
namespace OsEngine.Indicators
{
    ...
    public class ZigZagCMO : Aindicator
    {
        ...
        {
            _seriesCMO.Values[index] = _CMO.DataSeries[0].Values[index];

            List<decimal> values = _CMO.DataSeries[0].Values;
            ...
            List<decimal> valuesCMO = new List<decimal>();

            for (int i = 0; i < index + 1; i++)
            {
                valuesCMO.Add(values[i]);
            }

            decimal High = 0;
            decimal Low = 0;

            High = valuesCMO[valuesCMO.Count - 1];
            Low = valuesCMO[valuesCMO.Count - 1];
