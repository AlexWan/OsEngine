        ...
        public class MoexFixFastCurrencyServerRealization : IServerRealization
        {
            ...
            private void CreateFXMFIXTradeConnection(string fxMFIXTradeAddress, int fxMFIXTradePort)
            {
                ...
                if (_fxMFIXTradeTcpClient.Connected)
                {
                    ...
                    while (!_fxMFIXTradeConnectionSuccessful || waitStartConnect.AddSeconds(30) > DateTime.Now)
                    {
                        Thread.Sleep(2000);
           ...
           Dictionary<string, int> _minRptSeqFromTrades = new Dictionary<string, int>(); // минимальные значения полей RptSeq(83)
                        ...
                        case "A": // Logon
                            ...
                            if (newFixMsgNum > _incomingFixMsgNum)
                            {
                               ...
                               SendFXMFIXTradeMessage(resendMsg);
                                SendLogMessage($"Resend Request has been sent", LogMessageType.System);
                            }
                            else
                            {
                                _incomingFixMsgNum++;
            ...
            // список измененых ордеров, т.к. на бирже номер ордера меняется       
            ...
            private bool IsDataMissed(List<NumbersData> nums, out long beginMsgSeqNum, out long endMsgSeqNum, out int rptSeqMissedCount, out List<int> missedNums)
            {
            ...
            private StreamWriter _logIFXMFIXMsg = new StreamWriter($"Engine\\Log\\IncomingMFIX_{DateTime.Now:dd-MM-yyyy}.txt");
            ...
            private void WriteLogFXMFIXTrade(string fixMessage)
            {
                lock (_logLockMFIX)
                {
                    _logIFXMFIXMsg.WriteLine($"{DateTime.Now}: {fixMessage}");
                }
            }
