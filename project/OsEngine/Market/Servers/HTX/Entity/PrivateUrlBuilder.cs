...
namespace OsEngine.Market.Servers.HTX.Entity
{
    public class PrivateUrlBuilder
    {
        ...
        public string Build(string method, string path, DateTime utcDateTime, GetRequest request)
        {
            ...
            GetRequest req = new GetRequest(request);
            req.AddParam(_aKKey, _aKValue);
            req.AddParam(_sMKey, _sMVaue);
            req.AddParam(_sVKey, _sVValue);
            req.AddParam(_tKey, strDateTime);
