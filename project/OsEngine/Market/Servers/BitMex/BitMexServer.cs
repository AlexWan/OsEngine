...
namespace OsEngine.Market.Servers.BitMex
{
    ...
    public class BitMexServerRealization : IServerRealization
    {
            ...
            try
            {
                ...
                lock (_lockerStarter)
                {
                    ...
                    if (_subscribedSec.Find(s => s == namePaper) == null)
                    {
                        ...
                        string queryMyTrades =
                            "{\"op\": \"subscribe\", \"args\": [\"execution:" + security.Name + "\"]}";
            ...
            try
            {
                lock (_getCandles)
                {
                    ...
                    foreach (var bitMexCandle in allbmcandles)
                    {
                        ...
                        newCandle.TimeStart = Convert.ToDateTime(bitMexCandle.timestamp)
                        ...
                        for (int i = 0; i < quotes.data.Count; i++)
                        {
                            ...
                            else // (quotes.data[i].side == "Buy")
                            {
                                ...
                                for (int j = 0; j < _depth.Bids.Count; j++)
                                {
                                    ...
                                    else if (j != _depth.Bids.Count - 1 && id > _depth.Bids[i].Id &&
                                             id < _depth.Bids[j + 1].Id)
                                    {
        ...
        private void _client_ErrorEvent(string error)
        {
            ...
            {
                _client_SendLogMessage("Цена ликвидации при таком объме выше чем текущая цена ордера. Уменьшите объём",
