    ...
    public class HuobiServerRealization : AServerRealization
    {
        ...
        public override void GetPortfolios()
        {
            ...
            foreach (var portfolio in Portfolios)
            {
                ...
                for (int i = 0; i < accountBalance.data.list.Length; i++)
                {
                    ...
                    pos.ValueBlocked = currentData.balance;
        ...
        public override void SendOrder(Order order)
        {
            ...
            {
                SendLogMessage($"Ордер № {order.NumberUser} успешно размещен.", LogMessageType.Trade);
            }
            else
            {
                SendLogMessage($"Ошибка размещения ордера № {order.NumberUser} : {orderResponse.errorMessage}", LogMessageType.Trade);
        ...
        public override void CanselOrder(Order order)
        {
            ...
            {
                SendLogMessage($"Ордер № {order.NumberUser} успешно отменен.", LogMessageType.Trade);
                order.State = OrderStateType.Cancel;
                OnOrderEvent(order);
            }
            else
            {
                SendLogMessage($"Ошибка отмены ордера № {order.NumberUser} : {cancelResponse.errorMessage}", LogMessageType.Trade);
            }
        }
