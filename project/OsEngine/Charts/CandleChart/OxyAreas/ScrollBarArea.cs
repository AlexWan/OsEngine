...
namespace OsEngine.Charts.CandleChart.OxyAreas
{
    public class ScrollBarArea : OxyArea
    {
        ...
        public void SetAllChartByScreenPoligon()
        {
            ...
            Action action = () =>
                {
                    ((CandleStickArea)main_area).BuildCandleSeries(my_candles);
                    ...
                    main_area.date_time_axis_X.Zoom(first_candle_value, last_candle_value);

                    List<double> max_min = new List<double>();

                    max_min = main_area.GetHighLow(true, first_candle_value, last_candle_value);

                    main_area.linear_axis_Y.Zoom(max_min[0], max_min[1]);

                    main_area.plot_model.InvalidatePlot(true);
                };

            main_area.plot_view.Dispatcher.Invoke(action);
        }
