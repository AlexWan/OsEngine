...
namespace OsEngine.Charts.CandleChart.OxyAreas
{
    public class OxyArea 
    {
        ...
        public List<AreaSeries>      area_series_list        = new List<AreaSeries>();
        ...
        public List<IndicatorSeria> indicator_series = new List<IndicatorSeria>();
        ...
        public OxyArea(OxyAreaSettings settings, OxyChartPainter owner)
        {
            ...
            linear_axis_Y = new LinearAxis()
            {
                ...
                EdgeRenderingMode = area_settings.EdgeRenderingMode,
               // AbsoluteMinimum = area_settings.AbsoluteMinimum,
            };
        ...
        public void AnnotationAdd(List<IChartElement> ann_elements_list)
        {
            if (ann_elements_list.Count == 0)
                return;

            foreach (var element in ann_elements_list.Where(x => x.UniqName == (string)Tag))
            {
                if (plot_model.Annotations.ToList().Exists(x => (string)x.Tag == element.UniqName))
                    plot_model.Annotations.Remove(plot_model.Annotations.ToList().Find(x => (string)x.Tag == element.UniqName));
        ...
        public  OxyPlot.Annotations.Annotation ElementToAnnotation(IChartElement element)
        {
            ...
            if (main_chart.axis_Y_type == "linear" || (string)Tag != "Prime")
            {
                ...
                if (indi_seria.IndicatorType == IndicatorChartPaintType.Line)
                {
                    ...
                    else
                    {
                        ...
                        indi_seria.IndicatorPoints = points.ToList();
                ...
                if (indi_seria.IndicatorType == IndicatorChartPaintType.Point)
                {
                    ...
                    else
                    {
                        ...
                        indi_seria.IndicatorScatterPoints = points.ToList();
        ...
        public List<double> GetHighLow(bool isPrime, double start, double end)
        {
            ...
            lock (area_locker)
            {
                if (area_series_list.Count > 0)
                {
                    foreach (var area_series in area_series_list)
                    {
        ...
        {
            foreach (var area in all_areas.Where(x => x.Tag != this.Tag && (string)x.Tag != "ScrollChart" && (string)x.Tag != "ControlPanel"))
            {
        ...
        public void Redraw()
        {
            Action action = () =>
            {
                ...
                foreach (var area_seria in area_series_list)
                {
