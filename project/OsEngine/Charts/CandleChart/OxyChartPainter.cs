    ...
    public class OxyChartPainter : IChartPainter
    {
        ...
        public void ClearDataPointsAndSizeValue()
        {
            for (int i = 0; i < all_areas.Count; i++)
            {
                ...
                {
                    ((CandleStickArea)all_areas[i]).Clear();
                }

                if (all_areas[i] is ScrollBarArea)
                {
                    ((ScrollBarArea)all_areas[i]).Clear();
                }

                if (all_areas[i] is IndicatorArea)
                {
                    ((IndicatorArea)all_areas[i]).Clear();
                }

                if (all_areas[i] is ControlPanelArea)
                {
                    ((ControlPanelArea)all_areas[i]).Clear();
                }
            }
        }
        ...
        public void ClearSeries()
        {
            foreach (var area in all_areas)
            {
                area.indicator_series = new List<IndicatorSeria>();
            }
        }
        ...
        public string CreateSeries(string areaName, IndicatorChartPaintType indicatorType, string name)
        {
            ...
            foreach(var area in all_areas)
            {
                if ((string)area.Tag == areaName && !area.indicator_series.Exists(x => x.SeriaName == name))
                {
                    area.indicator_series.Add(new_seria);
                }
            }
        ...
        {
            var area = all_areas.Find(x => (string)x.Tag == indicator.NameArea);

            if (area.indicator_series.Exists(x => x.SeriaName == indicator.NameSeries))
            {
                area.indicator_series.Remove(area.indicator_series.Find(x => x.SeriaName == indicator.NameSeries));
            }

            if (area is IndicatorArea && area.indicator_series.Count == 0)
            {
                ((IndicatorArea)area).Clear();
        ...
        public bool IndicatorIsCreate(string name)
        {
            foreach (var area in all_areas)
            {
                if (area.indicator_series.Exists(x => x.SeriaName == name))
                    return true;
            }
        ...
        public void ProcessCandles(List<Candle> history)
        {
            ...
            int all_series_count = 0;

            foreach (var area in all_areas.Where(x => x is IndicatorArea || (string)x.Tag == "Prime"))
            {
                all_series_count += area.indicator_series.Count;
            }

            if (all_series_count > 0)
            {
                foreach (var area in all_areas.Where(x => x is IndicatorArea || (string)x.Tag == "Prime"))
                {
                    foreach (var seria in area.indicator_series)
                    {
            ...
            if (main_chart.factor > 1)
            {
                skipper = (int)main_chart.factor;
            }

            foreach (var area in all_areas.Where(x => (string)x.Tag != "ControlPanel"))
            {
                area.my_candles = history;
            }

            main_chart.BuildCandleSeries();

            if (all_areas.Where(x => x is IndicatorArea) != null )
            { 
                main_chart.Calculate(time_frame_span, time_frame);
                DrawChart();
            }

            main_chart.AnnotationAdd(ann_elements_list);
        }
        ...
        public void ProcessIndicator(IIndicator indicator)
        {
            ...
            foreach (var area_x in all_areas)
            {
                all_series_count += area_x.indicator_series.Count;
            }
            ...
            if (all_series_count > 0)
            {
                if (indicator.ValuesToChart != null)
                {
                    ...
                    //{
                    //    var seria_indi = all_areas.Find(x => (string)x.Tag == indicator.NameArea).indicator_series.Find(x => x.SeriaName == indicator.NameSeries);
                    ...
                    MessageBox.Show("Новый чарт не поддерживает старые индикаторы, так как мне лень их все равно скоро выпилят");
                }

                else
                {
                    Aindicator indi = (Aindicator)indicator;
                    List<IndicatorDataSeries> series = indi.DataSeries;

                    bool skip = true;

                    for (int i = 0; series != null && i < series.Count; i++)
                    {
                        var seria_indi = all_areas.Find(x => (string)x.Tag == indicator.NameArea).indicator_series.Find(x => x.SeriaName == series[i].NameSeries);
            ...
            if (indicator.ValuesToChart != null)
            {
                ...
                for (int i = 0; i < val_list.Count; i++)
                {
                    ...
                    var seria = all_areas.Find(x => (string)x.Tag == indicator.NameArea).indicator_series.Find(x => x.SeriaName == name + i.ToString());
            ...
            for (int i = 0; i < indi_series.Count; i++)
            {
                ...
                var seria = all_areas.Find(x => (string)x.Tag == indicator.NameArea).indicator_series.Find(x => x.SeriaName == indi_series[i].NameSeries);
        ...
        private void Element_UpdeteEvent(IChartElement element)
        {
            ...
            var annotation = area.ElementToAnnotation(element);
        ...
        public void RePaintIndicator(IIndicator indicatorCandle)
        {
            ...
            if (!all_areas.Find(x => (string)x.Tag == indicatorCandle.NameArea).indicator_series.Exists(x => x.SeriaName == indicatorCandle.NameSeries))
            {
                var new_seria = new IndicatorSeria()
                {
                ...
                all_areas.Find(x => (string)x.Tag == indicatorCandle.NameArea).indicator_series.Add(new IndicatorSeria()
                {
                    AreaName = indicatorCandle.NameArea,
                    IndicatorType = indicatorCandle.TypeIndicator,
                    SeriaName = indicatorCandle.NameSeries
        ...
        public void StartPaintPrimeChart(System.Windows.Controls.Grid grid_chart, WindowsFormsHost host, System.Windows.Shapes.Rectangle rectangle)
        {
            ...
            foreach (var area in all_areas)
            {
                area.AnnotationAdd(ann_elements_list);
            }
        ...
        public void MakeChart(System.Windows.Controls.Grid grid_chart)
        {
            ...
            OxyArea main_chart = all_areas.Find(x => (string)x.Tag == "Prime");

            if (main_chart == null)
            {
                main_chart = new CandleStickArea(new OxyAreaSettings()
                {
                    cursor_X_is_active = true,
                    cursor_Y_is_active = true,
                    Tag = "Prime",
            ...
            ((CandleStickArea)main_chart).Clear();
            ...
            if (all_areas.Exists(x => (string)x.Tag == "ScrollChart"))
            {
                ...
                ((ScrollBarArea)area_scroll).Clear();
            ...
            if (start_program != StartProgram.IsOsData)
            {
                ...
                if (all_areas.Exists(x => (string)x.Tag == "ControlPanel"))
                {
                    ...
                    ((ControlPanelArea)area_control).Clear();
        ...
        private void DrawChart()
        {
            ...
            List<IndicatorSeria> all_series = new List<IndicatorSeria>();

            foreach (var area in all_areas.Where(x => x is IndicatorArea || (string)x.Tag == "Prime"))
            {
                if (area.indicator_series.Count > 0)
                {
                    for (int i = 0; i < area.indicator_series.Count; i++)
                    {
                        all_series.Add(area.indicator_series[i]);
                    }
                }
            }


            foreach (var seria in all_series)
            {
